...
export class OnyriaActorSheet extends ActorSheet {
    ...
    activateListeners(html) {
        ...
        // Click to open
        html.find('.onyria-compendium-pack').click(ev => {
            ev.preventDefault();
        ...
        html.find('.item-create.new-item').click(ev => {
            ev.preventDefault();
            return OnyriaItem.createForActor(this.actor);
        });
   ...
   // const actorData = this.actor.toObject(false);
    ...
    async _updateCompValue() {
        ...
        comps_normal.forEach(element => {
            ...
            if(itemData.system.link1 == game.i18n.localize("ONYRIA.stats.str.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.str.base;
            }
            if(itemData.system.link1 == game.i18n.localize("ONYRIA.stats.dex.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.dex.base;
            }
            if(itemData.system.link1 == game.i18n.localize("ONYRIA.stats.end.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.end.base;
            }
            if(itemData.system.link1 == game.i18n.localize("ONYRIA.stats.int.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.int.base;
            }
            if(itemData.system.link1 == game.i18n.localize("ONYRIA.stats.cha.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.cha.base;
            }
            if(itemData.system.link1 == game.i18n.localize("ONYRIA.stats.intu.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.intu.base;
            }

            if(itemData.system.link2 == game.i18n.localize("ONYRIA.stats.str.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.str.base;
            }
            if(itemData.system.link2 == game.i18n.localize("ONYRIA.stats.dex.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.dex.base;
            }
            if(itemData.system.link2 == game.i18n.localize("ONYRIA.stats.end.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.end.base;
            }
            if(itemData.system.link2 == game.i18n.localize("ONYRIA.stats.int.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.int.base;
            }
            if(itemData.system.link2 == game.i18n.localize("ONYRIA.stats.cha.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.cha.base;
            }
            if(itemData.system.link2 == game.i18n.localize("ONYRIA.stats.intu.abbrev") )
            {
                itemData.system.score += this.actor.system.stats.intu.base;
            }
        ...
        switch (rolltype) {
            case "skillcheck" :
                ...
                if(globalSettingCarac100) 
                {
                    if(forceConfig)
                    {
                        ...
                        extraOptions = await this.getRollOptions("systems/onyria/templates/config/skill-options.hbs","Configuration du jet de caractéristique",configJet);
                        if (extraOptions.cancelled) return;     
                        extraOptions.multi = 1;                   
                    }
                    return OnyriaRoll.skillCheck(this.getData().system, this.actor, event,extraOptions.multi,extraOptions.rollType,extraOptions.bonusMalus);
                }
                else
                {
                    configJet.showBonus = true;
            ...
            case "competencycheck" :
                if(forceConfig)
                    {
                        configJet.showBonus = true;
                        extraOptions = await this.getRollOptions("systems/onyria/templates/config/skill-options.hbs","Configuration du jet de compétence",configJet);
                        if (extraOptions.cancelled) return;
                        if(extraOptions.bonusMalus == "0")
                            extraOptions.bonusMalus = "+0";
                    }
                    return OnyriaRoll.competencyCheck(this.getData().items, this.actor, event,extraOptions.bonusMalus,extraOptions.rollType);
                break;
            ...
            case "weapon" :
                ...
                return OnyriaRoll.rollWeapon(this.getData().system, this.actor, event,extraOptions.rollType);
            ...
            case "initiative" :
                ...
                {
                    configJet.checkGM = true;
                    extraOptions = await this.getRollOptions("systems/onyria/templates/config/skill-options.hbs","Configuration du jet d'initiative",configJet);
                    if (extraOptions.cancelled) return;
                }
    ...
    async _onDropItem(event, data) {
        ...
        switch (itemData.type) {
            ...
            default:
                ...
                let globalSettingMoveItem = game.settings.get("onyria","moveItem"); 
