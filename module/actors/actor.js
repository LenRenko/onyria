...
export class OnyriaActor extends Actor {
  ...
  static async create(data, options={}) {
    ...
    if (typeof data.items === 'undefined') {
      ...
      let caps = game.onyria.config.competences;
  ...
  async modifyTokenAttribute(attribute, value, isDelta, isBar) {
    ...
    getDefaultHand() {
      ...
      if (game.settings.get("onyria", "deckSetting").handsByActorID.hasOwnProperty(this.id)) {
          return game.cards.get(game.settings.get("onyria", "deckSetting").handsByActorID[this.id]);
  ...
  getDefaultDeck() {
    if (game.settings.get("onyria", "deckSetting").decksByActorID.hasOwnProperty(this.id)) {
        return game.cards.get(game.settings.get("onyria", "deckSetting").decksByActorID[this.id]);
  ...
  getDefaultDiscard() {
    if (game.settings.get("onyria", "deckSetting").discardsByActorID.hasOwnProperty(this.id)) {
        return game.cards.get(game.settings.get("onyria", "deckSetting").discardsByActorID[this.id]);
  ...
  async createDefaultCards() {
      let cartesMagie;
      cartesMagie = await game.packs.get("onyria.cartesmagiquesdonyria").getDocuments().then(index => index.map(entity => entity.toObject(false)));

      cartesMagie[0].name = "Deck de "+this.name;
      cartesMagie[0].ownership = this.getCardOwnership();

      // storing ids in game.settings
      let settingData = game.settings.get("onyria", "deckSetting");
      let actorId = this.id;
      ...
      game.settings.set("onyria", "deckSetting", settingData);
