export default class cardsSettingMenu extends FormApplication {
    ...
    static get defaultOptions() {
        ...
        options.template = "/systems/onyria/templates/cards/cardSettingMenu.hbs";
        options.top = 300;
        options.width = 1000;
    ...
    getData() {
        ...
        let data = {
            ...
            object: game.settings.get("onyria", "deckSetting"),
        ...
        for (let sk of data.decksByActorID) {
            if (game.settings.get("onyria", "deckSetting").decksByActorID) {
                data.decksByActorID[sk.id] = game.settings.get("onyria", "deckSetting").decksByActorID[sk.id];
            }
        }

        for (let sk of data.handsByActorID) {
            if (game.settings.get("onyria", "deckSetting").handsByActorID) {
                data.decksByActorID[sk.id] = game.settings.get("onyria", "deckSetting").handsByActorID[sk.id];
            }
        }

        for (let sk of data.discardsByActorID) {
            if (game.settings.get("onyria", "deckSetting").discardsByActorID) {
                data.decksByActorID[sk.id] = game.settings.get("onyria", "deckSetting").discardsByActorID[sk.id];
            }
        }
    ...
    _updateObject(event, formData) {
        if(event.submitter.name == "submit") //event.type almways submit even if cancel is clicked
        {
            ...
            //game.settings.set('onyria', 'deckSetting', data);
        }
    }
