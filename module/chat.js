 ...
 */
 export async function sortCustomAgeChatCards(chatCard, html, data) {
    if (chatCard.content.search("onyria-roll-chat") != -1)
    {
        const element = html.find(".message-content").html(chatCard.content);
    }

    if (html.find(".onyria-roll-chat").length > 0) _handleAgeRollVisibility(html, chatCard, data);
};
 ...
 */
 function _handleAgeRollVisibility(html, chatCard, chatData){
    const element = html.find(".onyria-roll-chat");
